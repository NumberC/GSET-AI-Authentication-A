...
% Minimum frequency to transmit is freq_min
% Max frequency to transmit is freq_max
% Single chirp duration in ms is chirp_time
% how_many_reps_per_signal is self explanatory
% file_name is self explanatory
function [person] = cross_correlate_attempt(fs, freq_min, freq_max, chirp_time, how_many_reps_per_signal, file_name)
    ...
    chirp_sig = audioread('chirp_50hzto500hz_16khzfs_125ms_100ms_Repeat10.wav');
    ...
    [x,~] = audioread(file_name); % read sequence from filename
    
    freq_range = [50 500];
    mic_a = bandpass(x(:,1), freq_range, fs);
    
    [acor_a, lag_a] = xcorr(chirp_sig, mic_a);
    ...
    % creating a person file is the responsibility of the caller, not of
    % this function
